...
function setupInputEvent(parentElement, selector) {
  ...
  if (element) {
    element.addEventListener("input", (event) => {
      ...
      // hourlyWageInput と totalWorkingHoursInput を掛け算する処理
      let hourlyWageInput = parentElement
        ? parentElement.querySelector(".hourlyWageInput")
        : document.querySelector(".hourlyWageInput");
      let totalWorkingHoursInput = parentElement
        ? parentElement.querySelector(".totalWorkingHoursInput")
        : document.querySelector(".totalWorkingHoursInput");
      let wageOutput = parentElement
        ? parentElement.querySelector(".wageOutput")
        : document.querySelector(".wageOutput");

      if (hourlyWageInput && totalWorkingHoursInput && wageOutput) {
        let hourlyWage = parseFloat(hourlyWageInput.value) || 0; // 数値に変換、無効な場合は0
        let totalWorkingHours = parseFloat(totalWorkingHoursInput.value) || 0; // 数値に変換、無効な場合は0
        let result = hourlyWage * totalWorkingHours; // 掛け算
        wageOutput.value = result.toFixed(2); // 修正後
        console.log(`合計金額: ${result}`); // 計算結果をコンソールに表示
      }
    });
  }
}
