  ...
  elements.forEach(({ selector, name }) => {
    ...
    if (element) {
      element.addEventListener("input", (event) =>
        handleInput(event, parentElement)
      );
    }
  });
}
...
function handleInput(event, parentElement) {
  ...
  const holidayDutyInput =
    parentElement?.querySelector(".holidayDutyInput") ||
    document.querySelector(".holidayDutyInput");
  const normalWagesElement =
    parentElement?.querySelector(".normalWages") ||
    document.querySelector(".normalWages");
  const wageOutput =
    parentElement?.querySelector(".wageOutput") ||
    document.querySelector(".wageOutput");
  const resultCompensation =
    parentElement?.querySelector(".resultCompensation") ||
    document.querySelector(".resultCompensation");
  ...
  // 結果の表示
  if (normalWagesElement)
    normalWagesElement.textContent = normalWages.toLocaleString();
  if (resultCompensation)
    resultCompensation.textContent = compensation.toLocaleString();
